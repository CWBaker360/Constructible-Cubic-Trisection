...
\hypersetup{
  ...
  pdftitle={A Constructible Cubic Error Cascade for Approximate Angle Trisection},
  pdfauthor={Wayne Baker},
  pdfsubject={Constructible approximation and cubic residual refinement for angle trisection},
  pdfkeywords={approximate angle trisection, constructible geometry, cubic error cascade}
}
...
\author{Wayne Baker}
\date{July 2, 2026\\[2pt]\small Version 1.0.0}

\begin{document}
\maketitle

\begin{center}
\small Research source archive: \url{https://github.com/CWBaker360/constructible-cubic-trisection}
\end{center}
...
Exact trisection of an arbitrary angle is impossible by straightedge and compass alone. This classical result, established by Wantzel, concerns exact Euclidean constructibility~\cite{Wantzel1837}; for broader historical context, see Dudley~\cite{Dudley1987}. The present paper addresses a different problem: the construction of high-order approximations to \(\theta/3\).

The seed construction and its first iterative improvement were analyzed in Rostamian's exposition of Baker's method~\cite{RostamianBaker}. The present paper formulates the mechanism as a residual operator, derives its local cubic law, and records the resulting finite-stage error cascade.
...
consistent with Wantzel's theorem~\cite{Wantzel1837}.
...
the same formal series composition by hand. A standalone version is supplied in
\texttt{scripts/verify\_series.py}.
...
Table~\ref{tab:numerical}. A standalone version is supplied in
\texttt{scripts/numerical\_consistency\_check.py}. The computation is not used as evidence for the
...
University of Maryland, Baltimore County, 2010; updated August 2025.
